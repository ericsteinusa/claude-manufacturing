...
def resolve_scan_url(raw: str) -> str | None:
    ...
    if rtype == "emp":
        return _lookup_emp_url(key)
    if rtype == "sup":
        return _lookup_sup_url(key)
    if rtype == "eq":
...
def _lookup_pms_url(key: str) -> str | None:
    if key.isdigit():
        return f"/maint/schedule/{key}/"
    return None


def _lookup_eq_url(key: str) -> str | None:
    if key.isdigit():
        return f"/maint/equipment/{key}/"
    return None


def _lookup_sup_url(key: str) -> str | None:
    if key.isdigit():
        return f"/qa/suppliers/{key}/"
    return None


def _lookup_emp_url(key: str) -> str | None:
    if key.isdigit():
        return f"/people/{key}/"
    return None
