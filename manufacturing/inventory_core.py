...
def _ensure_product_extra_columns(conn) -> None:
    """Self-heal: on a genuinely fresh Postgres deployment, ``product`` is
    created via schema.py's/work_orders_core.py's own
    ``CREATE TABLE IF NOT EXISTS`` — neither of which includes item_type,
    uom, lead_time_days, or created_by. Those are only added by
    seed_sample_products.py's own ALTER TABLE steps, a dev-only script
    never run in production. Callers that don't otherwise commit (e.g.
    create_product/update_product) rely on the caller's own commit to also
    persist these ALTERs, since Postgres DDL is transactional.
    """
    conn.execute(
