...
def init_db():
    ...
    try:
        conn.execute(
            "ALTER TABLE ar_invoice ADD COLUMN IF NOT EXISTS created_by TEXT")
    except Exception:
        pass
    conn.commit()
    conn.close()
...
class InvoiceDialog(QtWidgets.QDialog):
    ...
    def _build_ui(self):
        ...
        self.inv_num.setPlaceholderText("Invoice number")
    ...
    def _load(self):
        conn = get_db()
        rec = conn.execute("SELECT * FROM ar_invoice WHERE id = %s",
    ...
    self, "Input Error", "Invoice number is required.")
        ...
        try:
            if self._invoice_id is None:
                cur = conn.execute(
                    "INSERT INTO ar_invoice (customer_id, invoice_number, "
                    "invoice_date,"
                    " due_date, amount, description, status, created_by)"
                    " VALUES (%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id",
                    (self.customer_combo.currentData(), inv_num,
...
class RecordPaymentDialog(QtWidgets.QDialog):
    ...
    def _build_ui(self):
        ...
        self.reference.setPlaceholderText("Check #, wire ref, etc.")
    ...
    def _on_ok(self):
        ...
        paid = conn.execute(
            "SELECT COALESCE(SUM(amount),0) FROM ar_payment WHERE "
            "invoice_id=%s",
            (self._invoice_id,)
        ).fetchone()[0]
        inv = conn.execute(
            "SELECT amount FROM ar_invoice WHERE id=%s", (self._invoice_id,)
        ).fetchone()
        if inv:
            new_status = _invoice_status(inv["amount"], paid)
            conn.execute("UPDATE ar_invoice SET status=%s WHERE id=%s",
...
class AccountsReceivableWidget(QtWidgets.QWidget):
    ...
    def _build_ui(self):
        ...
        self.customer_filter.currentIndexChanged.connect(
            self._refresh_invoices)
    ...
    QtCore.QDate.currentDate().addMonths(3))
        ...
        self.inv_table.setHorizontalHeaderLabels(
            ["Invoice #", "Customer", "Invoice Date", "Due Date",
             "Amount", "Received", "Balance", "Status", "Created By"]
        )
    ...
    def _refresh_invoices(self):
        ...
        try:
            rows = conn.execute(f"""
                ...
                LEFT JOIN ar_payment p ON p.invoice_id = inv.id
                WHERE {where}
    ...
    row['first_name'] or ''} {
        row['last_name'] or ''}".strip()
            ...
            balance = row["amount"] - received
            total_amount += row["amount"]
            total_received += received
            self.inv_table.setItem(r, 0, _ro(row["invoice_number"]))
            self.inv_table.setItem(r, 1, _ro(customer_display))
            ...
            self.inv_table.setItem(r, 5, _ro_right(f"${received:,.2f}"))
    ...
    self, "No Selection", "Select an invoice first.")
        ...
        reply = QtWidgets.QMessageBox.question(
            self, "Confirm", f"Mark invoice as {new_status}?",
