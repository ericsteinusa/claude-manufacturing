...
class NewInvoiceDialog(QtWidgets.QDialog):
    ...
    def _build_ui(self):
        ...
        self.gl_acct_combo = QtWidgets.QComboBox()
        self.gl_acct_combo.setStyleSheet(COMBO_STYLE)
    ...
    def _on_save(self):
        if self.vendor_combo.currentData() is None:
            QtWidgets.QMessageBox.warning(self, "Error", "Select a vendor.")
            return
...
class AccountsPayable(QtWidgets.QMainWindow):
    ...
    def _build_vendors_tab(self):
        ...
        self.vend_table.setAlternatingRowColors(True)
        self.vend_table.verticalHeader().setVisible(False)
        self.vend_table.clicked.connect(self._on_vend_row_clicked)
        layout.addWidget(self.vend_table, stretch=1)
        ...
        sr.addWidget(sl)
        self.vend_search = QtWidgets.QLineEdit()
        self.vend_search.setStyleSheet(INPUT_STYLE)
        self.vend_search.setFixedWidth(200)
        self.vend_search.returnPressed.connect(self._on_vend_search)
        sr.addWidget(self.vend_search)
        ...
        self.vf_phone = inp("Phone")
        self.vf_phone.setFixedWidth(130)
        self.vf_email = inp("Email")
        self.vf_addr = inp("Street address")
        self.vf_city = inp("City")
        self.vf_state = inp("ST")
        self.vf_state.setMaxLength(2)
        self.vf_state.setFixedWidth(44)
        self.vf_zip = inp("Zip")
        self.vf_zip.setFixedWidth(90)

        grid.addWidget(lbl("Vendor Name:"), 0, 0)
        grid.addWidget(self.vf_name, 0, 1, 1, 3)
        grid.addWidget(lbl("Contact:"), 0, 4)
        grid.addWidget(self.vf_contact, 0, 5)
        grid.addWidget(lbl("Phone:"), 1, 0)
        grid.addWidget(self.vf_phone, 1, 1)
        grid.addWidget(lbl("Email:"), 1, 2)
        grid.addWidget(self.vf_email, 1, 3, 1, 3)
        grid.addWidget(lbl("Address:"), 2, 0)
        grid.addWidget(self.vf_addr, 2, 1, 1, 3)
        grid.addWidget(lbl("City:"), 2, 4)
        grid.addWidget(self.vf_city, 2, 5)
        grid.addWidget(lbl("State:"), 3, 0)
        grid.addWidget(self.vf_state, 3, 1)
        grid.addWidget(lbl("Zip:"), 3, 2)
        grid.addWidget(self.vf_zip, 3, 3)
        layout.addWidget(fg)
    ...
    def _build_invoices_tab(self):
        ...
        def fl(t):
            ...
            widget.setStyleSheet(LABEL_STYLE)
            return widget
        self.inv_vend_filter = QtWidgets.QComboBox()
        self.inv_vend_filter.setStyleSheet(COMBO_STYLE)
        self.inv_vend_filter.setMinimumWidth(180)
        ...
        for lbl_t, widget in (("Vendor:", fl("Vendor:")),):
            pass  # placeholder
        fr.addWidget(fl("Vendor:"))
        fr.addWidget(self.inv_vend_filter)
    ...
    def _load_vendors(self, search=None):
        ...
        conn.close()
        self.vend_table.setRowCount(0)
        self._vendor_row_ids = []
        for row in rows:
            r = self.vend_table.rowCount()
            self.vend_table.insertRow(r)
    ...
    def _refresh_vendor_filters(self):
        ...
        for combo in (self.inv_vend_filter,):
            ...
            combo.clear()
            combo.addItem("(all vendors)", None)
            for v in vendors:
                combo.addItem(v["vendor_name"], v["id"])
    ...
    def _vend_clear(self):
        for w in (self.vf_name, self.vf_contact, self.vf_phone, self.vf_email,
                  self.vf_addr, self.vf_city, self.vf_state, self.vf_zip):
            w.clear()
    ...
    def _collect_vendor_form(self):
        ...
        if not name:
            QtWidgets.QMessageBox.warning(self, "Input Error", "Vendor name is required.")
            return None
    ...
    def _on_vend_update(self):
        ...
        if not rows:
            QtWidgets.QMessageBox.warning(self, "No Selection", "Select a vendor first.")
            return
    ...
    def _inv_show_all(self):
        self.inv_vend_filter.setCurrentIndex(0)
    ...
    def _build_expense_tab(self):
        ...
        self.exp_status_filter = QtWidgets.QComboBox()
        self.exp_status_filter.setStyleSheet(COMBO_STYLE)
        ...
        self.exp_cat_filter = QtWidgets.QComboBox()
        self.exp_cat_filter.setStyleSheet(COMBO_STYLE)
        ...
        self.exp_table.setColumnWidth(3, 110)
        self.exp_table.setColumnWidth(4, 90)
        self.exp_table.setColumnWidth(5, 95)
        ...
        self.ef_cat = QtWidgets.QComboBox()
        self.ef_cat.setStyleSheet(COMBO_STYLE)
        self.ef_cat.addItems(EXP_CATEGORIES)
        self.ef_amt = QtWidgets.QDoubleSpinBox()
        self.ef_amt.setStyleSheet(SPIN_STYLE)
        self.ef_amt.setRange(0, 999_999)
        self.ef_amt.setDecimals(2)
        self.ef_amt.setGroupSeparatorShown(True)
        self.ef_date = QtWidgets.QDateEdit(calendarPopup=True)
        self.ef_date.setStyleSheet(DATE_STYLE)
        self.ef_date.setDisplayFormat("yyyy-MM-dd")
        self.ef_date.setDate(QtCore.QDate.currentDate())
        self.ef_status = QtWidgets.QComboBox()
        self.ef_status.setStyleSheet(COMBO_STYLE)
        self.ef_status.addItems(EXP_STATUSES)
        self.ef_notes = inp("Notes")

        grid.addWidget(lbl("Submitted By:"), 0, 0)
        grid.addWidget(self.ef_by, 0, 1)
        grid.addWidget(lbl("Description:"), 0, 2)
        grid.addWidget(self.ef_desc, 0, 3)
        grid.addWidget(lbl("Category:"), 0, 4)
        grid.addWidget(self.ef_cat, 0, 5)
        grid.addWidget(lbl("Amount:"), 1, 0)
        grid.addWidget(self.ef_amt, 1, 1)
        grid.addWidget(lbl("Date:"), 1, 2)
        grid.addWidget(self.ef_date, 1, 3)
        grid.addWidget(lbl("Status:"), 1, 4)
        grid.addWidget(self.ef_status, 1, 5)
        grid.addWidget(lbl("Notes:"), 2, 0)
        grid.addWidget(self.ef_notes, 2, 1, 1, 5)
        layout.addWidget(fg)
