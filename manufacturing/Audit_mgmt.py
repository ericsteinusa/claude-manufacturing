...
def _seed(con):
    ...
    con.execute(
        "INSERT INTO audit_schedule (audit_name, audit_type, department, "
        "auditor, scheduled, status) VALUES (%s,%s,%s,%s,%s,%s)",
        ("Annual Financial Audit", "Financial", "Accounting",
         "External Auditor", today, "Scheduled")
    )
...
AUDIT_TYPES = [
    ...
    "Quality",
     "Other"]
DEPARTMENTS = ["Accounting", "Engineering", "HR", "IT", "Maintenance", "Marketing",  # noqa: E501
...
class AuditDialog(QtWidgets.QDialog):
    def __init__(self, parent=None, row_data=None):
        ...
        self.ef_type.addItems(AUDIT_TYPES)
        self.ef_type.setEditable(True)
        self.ef_dept = QtWidgets.QComboBox()
        self.ef_dept.addItems(DEPARTMENTS)
        self.ef_dept.setEditable(True)
        self.ef_auditor = QtWidgets.QLineEdit()
        self.ef_sched = QtWidgets.QDateEdit(calendarPopup=True)
        self.ef_sched.setDisplayFormat("yyyy-MM-dd")
        self.ef_sched.setDate(QtCore.QDate.currentDate())
        self.ef_comp = QtWidgets.QDateEdit(calendarPopup=True)
        self.ef_comp.setDisplayFormat("yyyy-MM-dd")
        self.ef_comp.setDate(QtCore.QDate.currentDate())
        self.ef_status = QtWidgets.QComboBox()
        self.ef_status.addItems(AUDIT_STATUSES)
...
class FindingDialog(QtWidgets.QDialog):
    def __init__(self, parent=None, row_data=None):
        ...
        self.ef_desc = QtWidgets.QTextEdit()
        self.ef_desc.setMaximumHeight(80)
        self.ef_sev = QtWidgets.QComboBox()
        self.ef_sev.addItems(SEVERITIES)
        self.ef_dept = QtWidgets.QComboBox()
        self.ef_dept.addItems(DEPARTMENTS)
        self.ef_dept.setEditable(True)
        ...
        self.ef_status = QtWidgets.QComboBox()
        self.ef_status.addItems(FINDING_STATUSES)
        self.ef_notes = QtWidgets.QLineEdit()
...
class CorrectiveActionDialog(QtWidgets.QDialog):
    def __init__(self, parent=None, row_data=None):
        ...
        self.ef_comp.setDate(QtCore.QDate.currentDate())
        self.ef_status = QtWidgets.QComboBox()
        self.ef_status.addItems(CA_STATUSES)
        self.ef_notes = QtWidgets.QLineEdit()
...
class AuditMgmtWidget(QtWidgets.QWidget):
    ...
    def _build_schedule_tab(self):
        ...
        self.sched_status_filter.currentIndexChanged.connect(
            self._refresh_schedule)
        ...
        self.sched_type_filter.currentIndexChanged.connect(
            self._refresh_schedule)
        ...
        self.sched_tbl.setColumnWidth(2, 100)
        self.sched_tbl.setColumnWidth(3, 120)
        self.sched_tbl.setColumnWidth(4, 120)
        self.sched_tbl.setColumnWidth(5, 95)
        self.sched_tbl.setColumnWidth(6, 90)
        self.sched_tbl.setSelectionBehavior(
    QtWidgets.QAbstractItemView.SelectionBehavior.SelectRows)
        self.sched_tbl.setEditTriggers(
    QtWidgets.QAbstractItemView.EditTrigger.NoEditTriggers)
        ...
        for lbl, slot in [("Add Audit", self._add_audit), ("Edit Audit", self._edit_audit),  # noqa: E501
                          ("Delete Audit", self._delete_audit), ("Mark Complete", self._mark_audit_complete)]:  # noqa: E501
    ...
    def _refresh_schedule(self):
        sf = self.sched_status_filter.currentText() if hasattr(
            self, 'sched_status_filter') else "All Statuses"
        tf = self.sched_type_filter.currentText() if hasattr(
            self, 'sched_type_filter') else "All Types"
        with _conn() as con:
        ...
        for row in rows:
            r = self.sched_tbl.rowCount()
            self.sched_tbl.insertRow(r)
            self.sched_tbl.setItem(
    r, 0, _ro(
    ...
    def _edit_audit(self, *_):
        ...
        with _conn() as con:
            rd = con.execute(
    "SELECT * FROM audit_schedule WHERE id=%s", (rid,)).fetchone()
        if not rd:
            return
        ...
        if dlg.exec() == QtWidgets.QDialog.DialogCode.Accepted:
            v = dlg.values()
            with _conn() as con:
                con.execute("UPDATE audit_schedule SET audit_name=%s,audit_type=%s,department=%s,auditor=%s,scheduled=%s,completed=%s,status=%s,notes=%s WHERE id=%s",  # noqa: E501
                            (v["audit_name"], v["audit_type"], v["department"], v["auditor"], v["scheduled"], v["completed"], v["status"], v["notes"], rid))  # noqa: E501
            self._refresh_schedule()
    ...
    def _delete_audit(self, *_):
        ...
        if QtWidgets.QMessageBox.question(
            self, "Delete", "Delete this audit%s") == QtWidgets.QMessageBox.StandardButton.Yes:  # noqa: E501
            with _conn() as con:
    ...
    def _mark_audit_complete(self, *_):
        ...
        with _conn() as con:
            con.execute(
    "UPDATE audit_schedule SET status='Completed', completed=%s WHERE id=%s",
    (today,
     rid))
        self._refresh_schedule()
    ...
    def _build_findings_tab(self):
        ...
        self.find_sev_filter.currentIndexChanged.connect(
            self._refresh_findings)
        ...
        self.find_status_filter.currentIndexChanged.connect(
            self._refresh_findings)
        ...
        self.find_tbl.setColumnWidth(3, 90)
        self.find_tbl.setColumnWidth(4, 120)
        self.find_tbl.setColumnWidth(5, 95)
        self.find_tbl.setColumnWidth(6, 90)
        self.find_tbl.setSelectionBehavior(
    QtWidgets.QAbstractItemView.SelectionBehavior.SelectRows)
        self.find_tbl.setEditTriggers(
    QtWidgets.QAbstractItemView.EditTrigger.NoEditTriggers)
        ...
        bb = QtWidgets.QHBoxLayout()
        for lbl, slot in [("Add Finding", self._add_finding), ("Edit Finding", self._edit_finding),  # noqa: E501
                          ("Delete Finding", self._delete_finding), ("Mark Resolved", self._mark_finding_resolved)]:  # noqa: E501
            btn = QtWidgets.QPushButton(lbl)
            btn.setStyleSheet(BTN_STYLE)
            btn.clicked.connect(slot)
            bb.addWidget(btn)
    ...
    def _refresh_findings(self):
        sf = self.find_sev_filter.currentText() if hasattr(
            self, 'find_sev_filter') else "All Severities"
        stf = self.find_status_filter.currentText() if hasattr(
            self, 'find_status_filter') else "All Statuses"
        with _conn() as con:
        ...
        for row in rows:
            r = self.find_tbl.rowCount()
            self.find_tbl.insertRow(r)
            self.find_tbl.setItem(
    r, 0, _ro(
        row["id"], QtCore.Qt.AlignmentFlag.AlignRight))
    ...
    def _edit_finding(self, *_):
        ...
        with _conn() as con:
            rd = con.execute(
    "SELECT * FROM audit_finding WHERE id=%s", (rid,)).fetchone()
        if not rd:
            return
        ...
        if dlg.exec() == QtWidgets.QDialog.DialogCode.Accepted:
            v = dlg.values()
            with _conn() as con:
                con.execute("UPDATE audit_finding SET finding_ref=%s,description=%s,severity=%s,department=%s,found_date=%s,status=%s,notes=%s WHERE id=%s",  # noqa: E501
                            (v["finding_ref"], v["description"], v["severity"], v["department"], v["found_date"], v["status"], v["notes"], rid))  # noqa: E501
            self._refresh_findings()
            self._refresh_reports()
    ...
    def _delete_finding(self, *_):
        ...
        rid = int(self.find_tbl.item(self.find_tbl.currentRow(), 0).text())
        if QtWidgets.QMessageBox.question(
            self, "Delete", "Delete this finding%s") == QtWidgets.QMessageBox.StandardButton.Yes:  # noqa: E501
            with _conn() as con:
    ...
    def _mark_finding_resolved(self, *_):
        ...
        with _conn() as con:
            con.execute(
    "UPDATE audit_finding SET status='Resolved' WHERE id=%s", (rid,))
        self._refresh_findings()
    ...
    def _build_corrective_tab(self):
        ...
        self.ca_status_filter.currentIndexChanged.connect(
            self._refresh_corrective)
        ...
        self.ca_tbl.horizontalHeader().setSectionResizeMode(
            1, QtWidgets.QHeaderView.ResizeMode.Stretch)
        self.ca_tbl.setColumnWidth(2, 130)
        self.ca_tbl.setColumnWidth(3, 95)
        self.ca_tbl.setColumnWidth(4, 95)
        self.ca_tbl.setColumnWidth(5, 90)
        self.ca_tbl.setSelectionBehavior(
    QtWidgets.QAbstractItemView.SelectionBehavior.SelectRows)
        self.ca_tbl.setEditTriggers(
    QtWidgets.QAbstractItemView.EditTrigger.NoEditTriggers)
        ...
        bb = QtWidgets.QHBoxLayout()
        for lbl, slot in [("Add Action", self._add_ca), ("Edit Action", self._edit_ca),  # noqa: E501
                          ("Delete Action", self._delete_ca), ("Mark Complete", self._mark_ca_complete)]:  # noqa: E501
            btn = QtWidgets.QPushButton(lbl)
            btn.setStyleSheet(BTN_STYLE)
            btn.clicked.connect(slot)
            bb.addWidget(btn)
    ...
    def _refresh_corrective(self):
        sf = self.ca_status_filter.currentText() if hasattr(
            self, 'ca_status_filter') else "All Statuses"
        with _conn() as con:
        ...
        for row in rows:
            r = self.ca_tbl.rowCount()
            self.ca_tbl.insertRow(r)
            self.ca_tbl.setItem(
    r, 0, _ro(
        row["id"], QtCore.Qt.AlignmentFlag.AlignRight))
    ...
    def _edit_ca(self, *_):
        ...
        with _conn() as con:
            rd = con.execute(
    "SELECT * FROM corrective_action WHERE id=%s", (rid,)).fetchone()
        if not rd:
            return
        ...
        if dlg.exec() == QtWidgets.QDialog.DialogCode.Accepted:
            v = dlg.values()
            with _conn() as con:
                con.execute("UPDATE corrective_action SET description=%s,assigned_to=%s,due_date=%s,completed=%s,status=%s,notes=%s WHERE id=%s",  # noqa: E501
                            (v["description"], v["assigned_to"], v["due_date"], v["completed"], v["status"], v["notes"], rid))  # noqa: E501
            self._refresh_corrective()
            self._refresh_reports()
    ...
    def _delete_ca(self, *_):
        ...
        rid = int(self.ca_tbl.item(self.ca_tbl.currentRow(), 0).text())
        if QtWidgets.QMessageBox.question(
            self, "Delete", "Delete this corrective action%s") == QtWidgets.QMessageBox.StandardButton.Yes:  # noqa: E501
            with _conn() as con:
                con.execute(
    "DELETE FROM corrective_action WHERE id=%s", (rid,))
            self._refresh_corrective()
            self._refresh_reports()
    ...
    def _mark_ca_complete(self, *_):
        ...
        with _conn() as con:
            con.execute(
    "UPDATE corrective_action SET status='Completed', completed=%s WHERE id=%s",  # noqa: E501
    (today,
     rid))
        self._refresh_corrective()
    ...
    def _build_reports_tab(self):
        ...
        self.rpt_sched_tbl.horizontalHeader().setSectionResizeMode(
            0, QtWidgets.QHeaderView.ResizeMode.Stretch)
        self.rpt_sched_tbl.setColumnWidth(1, 60)
        self.rpt_sched_tbl.setEditTriggers(
    QtWidgets.QAbstractItemView.EditTrigger.NoEditTriggers)
        ...
        self.rpt_find_tbl.setHorizontalHeaderLabels(
            ["Severity", "Open", "Resolved"])
        self.rpt_find_tbl.horizontalHeader().setSectionResizeMode(
            0, QtWidgets.QHeaderView.ResizeMode.Stretch)
        self.rpt_find_tbl.setColumnWidth(1, 60)
        self.rpt_find_tbl.setColumnWidth(2, 70)
        self.rpt_find_tbl.setEditTriggers(
    QtWidgets.QAbstractItemView.EditTrigger.NoEditTriggers)
        ...
        self.rpt_ca_tbl.horizontalHeader().setSectionResizeMode(
            0, QtWidgets.QHeaderView.ResizeMode.Stretch)
        self.rpt_ca_tbl.setColumnWidth(1, 60)
        self.rpt_ca_tbl.setEditTriggers(
    QtWidgets.QAbstractItemView.EditTrigger.NoEditTriggers)
    ...
    def _refresh_reports(self):
        if not hasattr(self, 'rpt_sched_tbl'):
            return
        with _conn() as con:
            sched_rows = con.execute(
                "SELECT status, COUNT(*) as cnt FROM audit_schedule GROUP BY "
                "status").fetchall()
            find_rows = con.execute(
                "SELECT severity, SUM(CASE WHEN status IN ('Open','In "
                "Progress') THEN 1 ELSE 0 END) as open, "
                "SUM(CASE WHEN status IN ('Resolved','Closed') THEN 1 ELSE 0 "
                "END) as resolved "
                "FROM audit_finding GROUP BY severity ORDER BY CASE severity WHEN 'Critical' THEN 1 WHEN 'Major' THEN 2 WHEN 'Minor' THEN 3 ELSE 4 END"  # noqa: E501
            ).fetchall()
            ca_rows = con.execute(
                "SELECT status, COUNT(*) as cnt FROM corrective_action GROUP "
                "BY status").fetchall()

        self.rpt_sched_tbl.setRowCount(0)
        for row in sched_rows:
            r = self.rpt_sched_tbl.rowCount()
            self.rpt_sched_tbl.insertRow(r)
            self.rpt_sched_tbl.setItem(r, 0, _ro(row["status"]))
            self.rpt_sched_tbl.setItem(
                r, 1, _ro(str(row["cnt"]), QtCore.Qt.AlignmentFlag.AlignRight))
            _color_row(self.rpt_sched_tbl, r, STATUS_COLORS.get(row["status"]))

        self.rpt_find_tbl.setRowCount(0)
        for row in find_rows:
            r = self.rpt_find_tbl.rowCount()
            self.rpt_find_tbl.insertRow(r)
            self.rpt_find_tbl.setItem(r, 0, _ro(row["severity"]))
            self.rpt_find_tbl.setItem(
                r, 1, _ro(str(row["open"]), QtCore.Qt.AlignmentFlag.AlignRight))  # noqa: E501
            self.rpt_find_tbl.setItem(
                r, 2, _ro(str(row["resolved"]), QtCore.Qt.AlignmentFlag.AlignRight))  # noqa: E501
            _color_row(
    self.rpt_find_tbl,
    r,
    SEVERITY_COLORS.get(
        ...
        for row in ca_rows:
            r = self.rpt_ca_tbl.rowCount()
            self.rpt_ca_tbl.insertRow(r)
            self.rpt_ca_tbl.setItem(r, 0, _ro(row["status"]))
            self.rpt_ca_tbl.setItem(
                r, 1, _ro(str(row["cnt"]), QtCore.Qt.AlignmentFlag.AlignRight))
