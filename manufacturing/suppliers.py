...
class SupplierDetailPanel(QtWidgets.QWidget):
    ...
    def _build_ui(self):
        ...
        self.v_company = val(); self.v_contact = val()
        self.v_email   = val(); self.v_phone   = val()
        self.v_address = val(); self.v_pos     = val()

        layout.addWidget(lbl("Company:"),  0, 0); layout.addWidget(self.v_company, 0, 1)
        layout.addWidget(lbl("Contact:"),  0, 2); layout.addWidget(self.v_contact, 0, 3)
        layout.addWidget(lbl("Email:"),    1, 0); layout.addWidget(self.v_email,   1, 1)
        layout.addWidget(lbl("Phone:"),    1, 2); layout.addWidget(self.v_phone,   1, 3)
        layout.addWidget(lbl("Address:"),  2, 0); layout.addWidget(self.v_address, 2, 1, 1, 3)
        layout.addWidget(lbl("Open POs:"), 3, 0); layout.addWidget(self.v_pos,     3, 1)
