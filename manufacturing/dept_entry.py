...
root = Tk()
root.title('Department Entry Form')
app_width = 850
app_height = 600
...
def query_database():
	...
	c.execute("SELECT rowid, * FROM dept")
	records = c.fetchall()
...
def search_records():
	...
	c.execute("SELECT rowid, * FROM dept WHERE dept_name like ?", (lookup_record,))
	records = c.fetchall()
...
def lookup_records():
	...
	# Create label frame
	search_frame = LabelFrame(search, text="Department Name")
...
# Define Our Columns
my_tree['columns'] = ("Department Name", "ID")

# Format Our Columns
my_tree.column("#0", width=0, stretch=NO)
my_tree.column("Department Name", anchor=W, width=180)
my_tree.column("ID", anchor=CENTER, width=100)

# Create Headings
my_tree.heading("#0", text="", anchor=W)
my_tree.heading("Department Name", text="Department Name", anchor=W)
...
dn_label = Label(data_frame, text="Department Name")
dn_label.grid(row=0, column=0, padx=10, pady=10)
dn_entry = Entry(data_frame)
dn_entry.grid(row=0, column=1, padx=10, pady=10)
...
def remove_many():
	...
	if response == 1:
		...
		c.executemany("DELETE FROM dept WHERE id = ?", [(a,) for a in ids_to_delete])
...
def update_record():
	...
	selected = my_tree.focus()
	# Update record
	my_tree.item(selected, text="", values=(dn_entry.get(), id_entry.get()))
	...
	c.execute("""UPDATE dept SET
		...
		WHERE oid = :oid""",
		{
			'department_name': dn_entry.get(),
			'dept_id': id_entry.get(),
			'oid': id_entry.get(),
		})
...
def add_record():
	...
	# Add New Record
	c.execute("INSERT INTO dept (department_name) VALUES (?)", (dn_entry.get(),))
