...
def _seed_calendar(con):
    ...
    yr = today.year
    entries = [
        ("Federal Income Tax", "Quarterly estimated payment Q1",
         f"{yr}-04-15", f"Q1 {yr}", "Pending", ""),
        ("Federal Income Tax", "Quarterly estimated payment Q2",
         f"{yr}-06-15", f"Q2 {yr}", "Pending", ""),
        ("Federal Income Tax", "Quarterly estimated payment Q3",
         f"{yr}-09-15", f"Q3 {yr}", "Pending", ""),
        ("Federal Income Tax", "Quarterly estimated payment Q4",
         f"{yr + 1}-01-15", f"Q4 {yr}", "Pending", ""),
        ("Payroll Tax (941)", "Monthly deposit", f"{yr}-{today.month:02d}-15",
         f"{today.strftime('%b %Y')}", "Pending", ""),
        ("State Income Tax", "Annual filing",
         f"{yr}-04-15", f"FY {yr - 1}", "Pending", ""),
        ("Sales Tax",
    "Monthly filing",
    f"{yr}-{today.month:02d}-20",
    f"{today.strftime('%b %Y')}",
    "Pending",
     ""),
    ]
    con.executemany(
        "INSERT INTO tax_calendar (tax_type, description, due_date, period, "
        "status, notes) VALUES (%s,%s,%s,%s,%s,%s)",
        entries,
    )
...
TAX_TYPES = ["Federal Income Tax", "State Income Tax", "Payroll Tax (941)", "Sales Tax",  # noqa: E501
    ...
    def __init__(self, parent=None, row_data=None):
        ...
        if row_data:
            ...
            if row_data["due_date"]:
                self.ef_due_date.setDate(
    QtCore.QDate.fromString(
        row_data["due_date"], "yyyy-MM-dd"))
            if row_data["filed_date"]:
                self.ef_filed_date.setDate(
    QtCore.QDate.fromString(
        row_data["filed_date"],
         "yyyy-MM-dd"))
            self.ef_status.setCurrentText(row_data["status"])
...
class TaxMgmtWidget(QtWidgets.QWidget):
    ...
    def _build_calendar_tab(self):
        ...
        self.cal_status_filter.currentIndexChanged.connect(
            self._refresh_calendar)
        ...
        self.cal_type_filter.currentIndexChanged.connect(
            self._refresh_calendar)
        ...
        self.cal_tbl.horizontalHeader().setSectionResizeMode(
        ...
        self.cal_tbl.setSelectionBehavior(
    QtWidgets.QAbstractItemView.SelectionBehavior.SelectRows)
        self.cal_tbl.setEditTriggers(
    ...
    def _refresh_calendar(self):
        status_f = self.cal_status_filter.currentText() if hasattr(
            self, 'cal_status_filter') else "All Statuses"
        type_f = self.cal_type_filter.currentText() if hasattr(
            self, 'cal_type_filter') else "All Types"
        with _conn() as con:
            q = (
                "SELECT id, tax_type, description, due_date, period, status "
                "FROM tax_calendar WHERE 1=1"
            )
            params = []
    ...
    def _add_calendar_entry(self, *_):
        ...
        if dlg.exec() == QtWidgets.QDialog.DialogCode.Accepted:
            ...
            with _conn() as con:
                con.execute(
                    "INSERT INTO tax_calendar (tax_type, description, "
                    "due_date, period, status, notes) VALUES "
                    "(%s,%s,%s,%s,%s,%s)",
                    (v["tax_type"], v["description"], v["due_date"],
                     v["period"], v["status"], v["notes"])
                )
            self._refresh_calendar()
    ...
    def _edit_calendar_entry(self, *_):
        ...
        with _conn() as con:
            rd = con.execute(
    "SELECT * FROM tax_calendar WHERE id=%s", (row_id,)).fetchone()
        if not rd:
            return
        ...
        if dlg.exec() == QtWidgets.QDialog.DialogCode.Accepted:
            ...
            with _conn() as con:
                con.execute(
                    "UPDATE tax_calendar SET tax_type=%s, description=%s, "
                    "due_date=%s, period=%s, status=%s, notes=%s WHERE id=%s",
                    (v["tax_type"], v["description"], v["due_date"],
                     v["period"], v["status"], v["notes"], row_id)
                )
            self._refresh_calendar()
    ...
    def _delete_calendar_entry(self, *_):
        ...
        if QtWidgets.QMessageBox.question(
            self, "Delete", "Delete this calendar entry%s") == QtWidgets.QMessageBox.StandardButton.Yes:  # noqa: E501
            with _conn() as con:
    ...
    def _mark_cal_completed(self, *_):
        ...
        with _conn() as con:
            con.execute(
    "UPDATE tax_calendar SET status='Completed' WHERE id=%s", (row_id,))
        self._refresh_calendar()
    ...
    def _build_filing_tab(self):
        ...
        self.fil_status_filter.currentIndexChanged.connect(
            self._refresh_filings)
    ...
    QtWidgets.QAbstractItemView.EditTrigger.NoEditTriggers)
        ...
        self.fil_total_lbl = QtWidgets.QLabel(
            "Total Due: $0.00  |  Total Paid: $0.00  |  Balance: $0.00")
    ...
    def _refresh_filings(self):
        status_f = self.fil_status_filter.currentText() if hasattr(
            self, 'fil_status_filter') else "All Statuses"
        type_f = self.fil_type_filter.currentText() if hasattr(
            self, 'fil_type_filter') else "All Types"
        with _conn() as con:
    ...
    r, 0, _ro(
        ...
        self.fil_total_lbl.setText(
            f"Total Due: ${
    total_due:,.2f}  |  Total Paid: ${
        total_paid:,.2f}  |  Balance: ${
            bal:,.2f}"
        )
    ...
    def _edit_filing(self, *_):
        ...
        with _conn() as con:
            rd = con.execute(
    "SELECT * FROM tax_filing WHERE id=%s", (row_id,)).fetchone()
        if not rd:
            return
        ...
        if dlg.exec() == QtWidgets.QDialog.DialogCode.Accepted:
            ...
            with _conn() as con:
                con.execute(
                    "UPDATE tax_filing SET tax_type=%s,jurisdiction=%s,period=%s,amount_due=%s,amount_paid=%s,due_date=%s,filed_date=%s,status=%s,reference=%s,notes=%s WHERE id=%s",  # noqa: E501
                    (v["tax_type"], v["jurisdiction"], v["period"], v["amount_due"], v["amount_paid"],  # noqa: E501
                     v["due_date"], v["filed_date"], v["status"], v["reference"], v["notes"], row_id)  # noqa: E501
                )
            self._refresh_filings()
            self._refresh_reports()
    ...
    def _delete_filing(self, *_):
        ...
        row_id = int(self.fil_tbl.item(self.fil_tbl.currentRow(), 0).text())
        if QtWidgets.QMessageBox.question(
            self, "Delete", "Delete this filing%s") == QtWidgets.QMessageBox.StandardButton.Yes:  # noqa: E501
            with _conn() as con:
    ...
    def _mark_filed(self, *_):
        ...
        with _conn() as con:
            con.execute(
    "UPDATE tax_filing SET status='Filed', filed_date=%s WHERE id=%s",
    (today,
     row_id))
        self._refresh_filings()
        self._refresh_reports()
    ...
    def _build_payment_tab(self):
        ...
        self.pay_type_filter.currentIndexChanged.connect(
            self._refresh_payments)
        ...
        self.pay_method_filter.currentIndexChanged.connect(
            self._refresh_payments)
    ...
    def _refresh_payments(self):
        type_f = self.pay_type_filter.currentText() if hasattr(
            self, 'pay_type_filter') else "All Types"
        method_f = self.pay_method_filter.currentText() if hasattr(
            self, 'pay_method_filter') else "All Methods"
        with _conn() as con:
    ...
    def _edit_payment(self, *_):
        ...
        with _conn() as con:
            rd = con.execute(
    "SELECT * FROM tax_payment WHERE id=%s", (row_id,)).fetchone()
        if not rd:
            return
        ...
        if dlg.exec() == QtWidgets.QDialog.DialogCode.Accepted:
            ...
            with _conn() as con:
                con.execute(
                    "UPDATE tax_payment SET tax_type=%s,jurisdiction=%s,period=%s,amount=%s,payment_date=%s,method=%s,reference=%s,notes=%s WHERE id=%s",  # noqa: E501
                    (v["tax_type"], v["jurisdiction"], v["period"], v["amount"],  # noqa: E501
                     v["payment_date"], v["method"], v["reference"], v["notes"], row_id)  # noqa: E501
                )
            self._refresh_payments()
            self._refresh_reports()
    ...
    def _delete_payment(self, *_):
        ...
        row_id = int(self.pay_tbl.item(self.pay_tbl.currentRow(), 0).text())
        if QtWidgets.QMessageBox.question(
            self, "Delete", "Delete this payment%s") == QtWidgets.QMessageBox.StandardButton.Yes:  # noqa: E501
            with _conn() as con:
    ...
    def _build_reports_tab(self):
        ...
        self.rpt_filing_tbl.setHorizontalHeaderLabels(
            ["Tax Type", "Count", "Total Due", "Total Paid"])
        self.rpt_filing_tbl.horizontalHeader().setSectionResizeMode(
        ...
        self.rpt_filing_tbl.setEditTriggers(
    QtWidgets.QAbstractItemView.EditTrigger.NoEditTriggers)
        ...
        self.rpt_payment_tbl.setHorizontalHeaderLabels(
            ["Tax Type", "Count", "Total Paid"])
        self.rpt_payment_tbl.horizontalHeader().setSectionResizeMode(
            0, QtWidgets.QHeaderView.ResizeMode.Stretch)
        self.rpt_payment_tbl.setColumnWidth(1, 55)
        self.rpt_payment_tbl.setColumnWidth(2, 100)
        self.rpt_payment_tbl.setEditTriggers(
    QtWidgets.QAbstractItemView.EditTrigger.NoEditTriggers)
        ...
        v.addWidget(QtWidgets.QLabel(
            "<b>Upcoming Deadlines (next 60 days)</b>"))
        self.rpt_upcoming_tbl = QtWidgets.QTableWidget(0, 4)
        self.rpt_upcoming_tbl.setHorizontalHeaderLabels(
            ["Tax Type", "Description", "Due Date", "Status"])
        self.rpt_upcoming_tbl.horizontalHeader().setSectionResizeMode(
            1, QtWidgets.QHeaderView.ResizeMode.Stretch)
        ...
        self.rpt_upcoming_tbl.setEditTriggers(
    QtWidgets.QAbstractItemView.EditTrigger.NoEditTriggers)
    ...
    def _refresh_reports(self):
        ...
        with _conn() as con:
            fil_rows = con.execute(
                "SELECT tax_type, COUNT(*) as cnt, SUM(amount_due) as due, "
                "SUM(amount_paid) as paid FROM tax_filing GROUP BY tax_type "
                "ORDER BY tax_type"
            ).fetchall()
            pay_rows = con.execute(
                "SELECT tax_type, COUNT(*) as cnt, SUM(amount) as total FROM "
                "tax_payment GROUP BY tax_type ORDER BY tax_type"
            ).fetchall()
            today = date.today().isoformat()
            from datetime import timedelta
            cutoff = (date.today() + timedelta(days=60)).isoformat()
            upcoming = con.execute(
                "SELECT tax_type, description, due_date, status FROM "
                "tax_calendar WHERE due_date BETWEEN %s AND %s ORDER BY "
                "due_date",
                (today, cutoff)
            ).fetchall()
