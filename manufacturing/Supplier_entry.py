...
def query_database():
	...
	c.execute("SELECT rowid, * FROM supplier")
	records = c.fetchall()
...
def search_records():
	...
	c.execute("SELECT rowid, * FROM supplier WHERE last_name like ?", (lookup_record,))
	records = c.fetchall()
...
# Define Our Columns
my_tree['columns'] = ("ID", "First Name", "Last Name", "Company Name", "Phone", "Address", "City", "State", "Zipcode", "Email")
...
my_tree.column("Company Name", anchor=CENTER, width=100)
...
cn_label = Label(data_frame, text="Company Name")
cn_label.grid(row=0, column=4, padx=10, pady=10)
cn_entry = Entry(data_frame)
cn_entry.grid(row=0, column=5, padx=10, pady=10)

ph_label = Label(data_frame, text="Phone Number")
...
address_label.grid(row=0, column=8, padx=10, pady=10)
address_entry = Entry(data_frame)
address_entry.grid(row=0, column=9, padx=10, pady=10)
...
zip_code_label.grid(row=1, column=4, padx=10, pady=10)
zip_code_entry = Entry(data_frame)
zip_code_entry.grid(row=1, column=5, padx=10, pady=10)

email_label = Label(data_frame, text="Email")
email_label.grid(row=1, column=6, padx=10, pady=10)
email_entry = Entry(data_frame)
email_entry.grid(row=1, column=7, padx=10, pady=10)
...
def remove_one():
	...
	c.execute("DELETE from supplier WHERE oid=" + id_entry.get())
...
def remove_many():
	...
	if response == 1:
		...
		c.executemany("DELETE FROM supplier WHERE id = ?", [(a,) for a in ids_to_delete])
...
def update_record():
	...
	selected = my_tree.focus()
	# Update record
	my_tree.item(selected, text="", values=(id_entry.get(), fn_entry.get(), ln_entry.get(), cn_entry.get(), ph_entry.get(), address_entry.get(), city_entry.get(), state_entry.get(), zip_code_entry.get(), email_entry.get()))
...
def add_record():
	...
	# Add New Record
	c.execute("INSERT INTO supplier (first_name, last_name, company_name, phone_number, address, city, state, zip_code, email) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (fn_entry.get(), ln_entry.get(), cn_entry.get(), ph_entry.get(), address_entry.get(), city_entry.get(), state_entry.get(), zip_code_entry.get(), email_entry.get()))
