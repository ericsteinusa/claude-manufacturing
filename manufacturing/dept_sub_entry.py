...
root = Tk()
root.title('Department Sub Entry Form')
app_width = 650
app_height = 600
...
def query_database():
	...
	c.execute("SELECT rowid, * FROM dept_sub")
	records = c.fetchall()
	...
	for record in records:
		if count % 2 == 0:
			my_tree.insert(parent='', index='end', iid=count, text='', values=(record[2], record[1]), tags=('evenrow',))
		else:
			my_tree.insert(parent='', index='end', iid=count, text='', values=(record[2], record[1]), tags=('oddrow',))
		# increment counter
		count += 1
...
def search_records():
	...
	c.execute("SELECT rowid, * FROM dept_sub WHERE dept_sub_name like ?", (lookup_record,))
	records = c.fetchall()
...
def lookup_records():
	...
	# Create label frame
	search_frame = LabelFrame(search, text="Department Sub Name")
...
# Create Table
c.execute("""CREATE TABLE if not exists dept_sub (
	dept_sub_name text,
	dept_sub_id integer)
	""")
...
# Define Our Columns
my_tree['columns'] = ("Department Sub Name", "ID")

# Format Our Columns
my_tree.column("#0", width=0, stretch=NO)
my_tree.column("Department Sub Name", anchor=W, width=210)
my_tree.column("ID", anchor=CENTER, width=100)

# Create Headings
my_tree.heading("#0", text="", anchor=W)
my_tree.heading("Department Sub Name", text="Department Sub Name", anchor=W)
...
dsn_label = Label(data_frame, text="Department Sub Name")
dsn_label.grid(row=0, column=0, padx=10, pady=10)
dsn_entry = Entry(data_frame)
dsn_entry.grid(row=0, column=1, padx=10, pady=10)
...
def remove_one():
	...
	c.execute("DELETE from dept WHERE oid=" + id_entry.get())
...
def remove_many():
	...
	if response == 1:
		...
		c.executemany("DELETE FROM dept_sub WHERE id = ?", [(a,) for a in ids_to_delete])
...
def update_record():
	...
	selected = my_tree.focus()
	# Update record
	my_tree.item(selected, text="", values=(dsn_entry.get(), id_entry.get()))
	...
	c.execute("""UPDATE dept SET
		dept_sub_name = :dept_sub_name,
		dept_sub_id = :dept_sub_id
	
		WHERE oid = :oid""",
		{
			'department_sub_name': dsn_entry.get(),
			'dept_sub_id': id_entry.get(),
			'oid': id_entry.get(),
		})
...
def add_record():
	...
	# Add New Record
	c.execute("INSERT INTO dept_sub (dept_sub_name) VALUES (?)", (dsn_entry.get(),))
