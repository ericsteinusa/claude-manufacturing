...
rather than failing silently. **Wired into four screens so far**: Time Clock
...
connectivity), and Quality's NCR list view (read-cache only — creating an NCR
still requires connectivity). The other 17 screens have no offline support yet —
